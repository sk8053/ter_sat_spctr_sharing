...
Y = np.zeros_like(Z)
X = np.repeat(10, len(Z))
uav_loc = np.column_stack((X, Y, Z))
...
cell_type = np.repeat([aerial_cell], len(dist_matrices))
...
for channel in channels:
    ...
    data_all.append(data['tx_elem_gain'])
    #print ('tx_elem_gain:  ', data['tx_elem_gain'])
    #print (channel.link_state)
    #print (channel.link_state, data['aoa_theta'], data['im']) #90-channel.ang[:,MPChan.aoa_theta_ind][0])
plt.scatter (X, Z, edgecolors='r', facecolors = 'r', marker='*', label = 'UAV')
plt.scatter (0, 0, edgecolors='b', facecolors = 'b', marker='^', label = 'BS')

r = np.round(data_all,2)

for i, (x, z, r0) in enumerate(zip (X[:-1], Z[:-1], r[:-1])):
    #print (x,z)
    plt.text(x,z, str(r0), fontsize = 12.5)
#plt.xlim(min(Z)-2.5,max(Z)+2.5)
plt.grid()
