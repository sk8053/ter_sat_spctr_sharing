...
class PPP_deployment():
    ...
    def get_terrestril_BS(self,lambda_t):
        ...
        xx_u, yy_u,zz_u = self.check_inter_distance_BS_UAV(
            self.xx_t, self.yy_t, self.zz_t, self.xx_u, self.yy_u, self.zz_u, self.args['min_dist']
        )
        self.num_uav = len(self.xx_u)
        return (xx_u, yy_u, zz_u), (self.xx_t, self.yy_t, self.zz_t)

    def get_aerial_BS(self, lambda_aeri):
        ## Aerial BS
        self.num_aeri = scipy.stats.poisson(lambda_aeri * self.areaTotal).rvs()  # Poisson number of points
        ...
        return (xx_u, yy_u, zz_u), (self.xx_a, self.yy_a, self.zz_a)
    ...
    def get_uav_to_delete(self):
        return self.uav_indices_to_delete
